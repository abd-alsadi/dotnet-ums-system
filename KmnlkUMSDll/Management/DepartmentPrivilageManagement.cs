...
{
    public class DepartmentPrivilageManagement :ModelManagement, ICURDOperations, IValidationOperations
    {
        private DepartmentPrivilageManager manager;
        private ILog logger;
        public DepartmentPrivilageManagement(ConnectionManager conn,ILog logger)
        {
            this.logger = logger;
            manager = new DepartmentPrivilageManager(conn, logger);  
        }
        ...
        public List<KmnlkUMSModel> GetByPrivilage(KmnlkUMSModel model)
        {
            ...
            try 
            {
                ...
                var result = manager.GetByPrivilage(model);
                logger.WriteToLog(EnvironmentManagement.getCurrentMethodName(this.GetType()), "", ENUM_TYPE_MSG_LOGGER.INFO,ENUM_TYPE_Block_LOGGER.END, modConstant.MSG_SUCCESS);
                return result;
            }
            catch (Exception e)
