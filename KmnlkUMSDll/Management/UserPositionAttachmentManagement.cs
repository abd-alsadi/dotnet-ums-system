...
{
    public class UserPositionAttachmentManagement :ModelManagement, ICURDOperations, IValidationOperations
    {
        private UserPositionAttachmentManager manager;
        private ILog logger;
        public UserPositionAttachmentManagement(ConnectionManager conn,ILog logger)
        {
            this.logger = logger;
            manager = new UserPositionAttachmentManager(conn, logger);  
        }
        ...
        public List<KmnlkUMSModel> GetByUserPosition(KmnlkUMSModel model)
        {
            ...
            try 
            {
                ...
                var result = manager.GetByUserPosition(model);
                logger.WriteToLog(EnvironmentManagement.getCurrentMethodName(this.GetType()), "", ENUM_TYPE_MSG_LOGGER.INFO,ENUM_TYPE_Block_LOGGER.END, modConstant.MSG_SUCCESS);
                return result;
            }
            catch (Exception e)
