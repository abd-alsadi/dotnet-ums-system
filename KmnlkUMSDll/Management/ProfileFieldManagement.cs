...
{
    public class ProfileFieldManagement :ModelManagement, ICURDOperations, IValidationOperations
    {
        private ProfileFieldManager manager;
        private ILog logger;
        public ProfileFieldManagement(ConnectionManager conn,ILog logger)
        {
            this.logger = logger;
            manager = new ProfileFieldManager(conn, logger);  
        }
        ...
        public List<KmnlkUMSModel> GetByProfile(KmnlkUMSModel model)
        {
            ...
            try 
            {
                ...
                var result = manager.GetByProfile(model);
                logger.WriteToLog(EnvironmentManagement.getCurrentMethodName(this.GetType()), "", ENUM_TYPE_MSG_LOGGER.INFO,ENUM_TYPE_Block_LOGGER.END, modConstant.MSG_SUCCESS);
                return result;
            }
            catch (Exception e)
