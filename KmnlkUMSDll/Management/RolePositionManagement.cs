...
{
    public class RolePositionManagement :ModelManagement, ICURDOperations, IValidationOperations
    {
        private RolePositionManager manager;
        private ILog logger;
        public RolePositionManagement(ConnectionManager conn,ILog logger)
        {
            this.logger = logger;
            manager = new RolePositionManager(conn, logger);  
        }
        ...
        public List<KmnlkUMSModel> GetByRole(KmnlkUMSModel model)
        {
            ...
            try 
            {
                ...
                var result = manager.GetByRole(model);
                logger.WriteToLog(EnvironmentManagement.getCurrentMethodName(this.GetType()), "", ENUM_TYPE_MSG_LOGGER.INFO,ENUM_TYPE_Block_LOGGER.END, modConstant.MSG_SUCCESS);
                return result;
            }
            catch (Exception e)
