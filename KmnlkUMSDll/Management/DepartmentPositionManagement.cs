...
{
    public class DepartmentPositionManagement : ModelManagement, ICURDOperations, IValidationOperations
    {
        private DepartmentPositionManager manager;
        private ILog logger;
        public DepartmentPositionManagement(ConnectionManager conn,ILog logger)
        {
            this.logger = logger;
            manager = new DepartmentPositionManager(conn, logger);  
        }
        ...
        public List<KmnlkUMSModel> GetByDepartment(KmnlkUMSModel model)
        {
            ...
            try 
            {
                ...
                var result = manager.GetByDepartment(model);
                logger.WriteToLog(EnvironmentManagement.getCurrentMethodName(this.GetType()), "", ENUM_TYPE_MSG_LOGGER.INFO,ENUM_TYPE_Block_LOGGER.END, modConstant.MSG_SUCCESS);
                return result;
            }
            catch (Exception e)
        ...
        public List<KmnlkUMSModel> GetByPosition(KmnlkUMSModel model)
        {
            ...
            try 
            {
                ...
                var result = manager.GetByPosition(model);
                logger.WriteToLog(EnvironmentManagement.getCurrentMethodName(this.GetType()), "", ENUM_TYPE_MSG_LOGGER.INFO,ENUM_TYPE_Block_LOGGER.END, modConstant.MSG_SUCCESS);
                return result;
            }
            catch (Exception e)
