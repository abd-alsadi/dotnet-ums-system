...
{
    public class DelegationManagement : ModelManagement, ICURDOperations, IValidationOperations
    {
        private DelegationManager manager;
        private ILog logger;
        public DelegationManagement(ConnectionManager conn,ILog logger)
        {
            this.logger = logger;
            manager = new DelegationManager(conn, logger);  
        }
        ...
        public List<KmnlkUMSModel> GetByFromUser(KmnlkUMSModel model)
        {
            ...
            try 
            {
                ...
                var result = manager.GetByFromUser(model);
                logger.WriteToLog(EnvironmentManagement.getCurrentMethodName(this.GetType()), "", ENUM_TYPE_MSG_LOGGER.INFO,ENUM_TYPE_Block_LOGGER.END, modConstant.MSG_SUCCESS);
                return result;
            }
            catch (Exception e)
        ...
        public List<KmnlkUMSModel> GetByToUser(KmnlkUMSModel model)
        {
            ...
            try 
            {
                ...
                var result = manager.GetByToUser(model);
                logger.WriteToLog(EnvironmentManagement.getCurrentMethodName(this.GetType()), "", ENUM_TYPE_MSG_LOGGER.INFO,ENUM_TYPE_Block_LOGGER.END, modConstant.MSG_SUCCESS);
                return result;
            }
            catch (Exception e)
