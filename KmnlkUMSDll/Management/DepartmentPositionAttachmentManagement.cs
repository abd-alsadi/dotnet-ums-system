...
{
    public class DepartmentPositionAttachmentManagement :ModelManagement, ICURDOperations, IValidationOperations
    {
        private DepartmentPositionAttachmentManager manager;
        private ILog logger;
        public DepartmentPositionAttachmentManagement(ConnectionManager conn,ILog logger)
        {
            this.logger = logger;
            manager = new DepartmentPositionAttachmentManager(conn, logger);  
        }
        ...
        public List<KmnlkUMSModel> GetByDepartmentPosition(KmnlkUMSModel model)
        {
            ...
            try 
            {
                ...
                var result = manager.GetByDepartmentPosition(model);
                logger.WriteToLog(EnvironmentManagement.getCurrentMethodName(this.GetType()), "", ENUM_TYPE_MSG_LOGGER.INFO,ENUM_TYPE_Block_LOGGER.END, modConstant.MSG_SUCCESS);
                return result;
            }
            catch (Exception e)
